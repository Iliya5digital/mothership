...
const update_host_tile = (elem, mac, host) => {
    ...
    if (host.config !== null) {
        html += `<div class="tile-title">${host.config.name}</div>`;
        elem.classList.add("known");
    } else {
        html += `<div>Not in config</div>`;
    }
    html += `<div class="mono">${mac.toUpperCase()}</div>`;
...
export const update_host_window = () => {
    ...
    if (host !== undefined && host.config !== null) {
        html += `<h1>${host.config.name}</h1>`;
    }
    html += `<h2 class="mono">${mac.toUpperCase()}</h2>`;
...
const ioc_block = (host) => {
    ...
    if (host.status.ioc !== undefined) {
        ...
        html += `<div>Statistics: <div class="code">${JSON.stringify(host.status.ioc.stats, null, 2)}</div></div>`;
    } else {
